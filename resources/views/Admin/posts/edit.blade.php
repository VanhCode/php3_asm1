                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="live-preview">
                                <form action="{{ route('posts.update', $postById->id) }}" id="formSubmit" enctype="multipart/form-data" method="POST">
                                    @csrf
                                    @method('POST')
                                    <div class="mb-3">
                                        <label for="employeeName" class="form-label">Tiêu đề</label>
                                        <input type="text" class="form-control" value="{{ $postById->title }}"
                                               name="title" id="title"
                                               placeholder="Nhập tiêu đề">
                                    </div>
                                    <div class="mb-3">
                                        <label for="employeeName" class="form-label">Slug</label>
                                        <input type="text" class="form-control" value="{{ $postById->slug }}"
                                    ...
                                    <div class="mb-3 w-25">
                                        <img src="{{ $postById->image }}" id="img" width="300px" height="300px" alt=""
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select w-25" id="category_id" name="category_id">
                                            @foreach($dataDanhMuc as $category)
                                                <option {{ $category->id == $postById->category_id ? 'selected' : '' }} value="{{ $category->id }}">{{ $category->category_name }}</option>
                                                @if(count($category->childrenRecursive) > 0)
                                                    @include('Admin.components.child-category', [
                                                        'children' => $category->childrenRecursive,
                                                        'depth' => 1,
                                                        'categoryCheck' => $postById->category_id
                                                    ])
                                                @endif
                                            @endforeach
                                        </select>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#formSubmit').on('submit', function (e) {
                ...
                $.ajax({
                    url: '{{ route("posts.update", ['id' => $postById->id]) }}',
                    type: 'POST',
                    data: formData,
                    contentType: false,
                    processData: false,
